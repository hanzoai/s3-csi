...
import (
	...
	"github.com/hanzoai/s3-csi/pkg/driver"
...
var (
	...
	driverName     = flag.String("driverName", "s3-csi", "CSI driver name, used by CSIDriver and StorageClass")

	filer             = flag.String("filer", "localhost:8888", "filer server")
	endpoint          = flag.String("endpoint", "unix://tmp/s3-csi.sock", "CSI endpoint to accept gRPC calls")
	mountEndpoint     = flag.String("mountEndpoint", "unix:///tmp/s3-mount.sock", "mount service endpoint")
...
func main() {
	...
	drv := driver.NewS3Driver(*driverName, *filer, *nodeID, *endpoint, *mountEndpoint, *enableAttacher)
