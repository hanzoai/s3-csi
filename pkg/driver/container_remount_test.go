...
100 50 0:55 / /mnt/s3 rw,nosuid,nodev,relatime - fuse.s3 s3 rw,user_id=0,group_id=0
101 50 0:56 / /data rw,nosuid,nodev,relatime - fuse s3 rw,user_id=0,group_id=0
`
	tmpDir := t.TempDir()
	...
	entries := []mountInfoEntry{
		...
		{device: "0:55", mountpoint: "/mnt/s3", fstype: "fuse.s3"},
	...
	if !isFuseFS(entries[2].fstype) {
		t.Error("fuse.s3 should be fuse")
	}
...
func TestParseMountInfoReaderCapturesRoot(t *testing.T) {
	...
	content := `100 50 0:55 / /var/lib/kubelet/pods/uid/volumes/kubernetes.io~csi/pv/mount rw,relatime - fuse.s3 s3 rw,user_id=0,group_id=0
101 50 0:55 /store /var/lib/kubelet/pods/uid/volume-subpaths/pv/ctr/0 rw,nosuid,nodev,relatime - fuse.s3 s3 rw,user_id=0,group_id=0
`
...
func TestParseMountInfoReaderUnescapesOctal(t *testing.T) {
	...
	content := "200 50 0:55 /my\\040store /var/lib/kubelet/pods/uid/a\\040b rw,relatime - fuse.s3 s3 rw\n"
	entries, err := parseMountInfoReader(strings.NewReader(content))
	if err != nil {
		t.Fatalf("parseMountInfoReader: %v", err)
	}
