...
func newNodeServerWithFakes(t *testing.T, state *fakeMountState) *NodeServer {
	...
	ns := &NodeServer{
		...
		mounterFactory: func(volumeID string, readOnly bool, driver *S3Driver, volContext map[string]string) (Mounter, error) {
...
// s3/s3-csi#253. It walks through the full CSI
...
func TestHealthMonitorRecoversStaleMount(t *testing.T) {
	...
	// dead path. Regression test for s3/s3-csi#261.
...
func TestHealthMonitorPreservesPublishesOnReStageFailure(t *testing.T) {
	...
	ns.mounterFactory = func(volumeID string, readOnly bool, driver *S3Driver, volContext map[string]string) (Mounter, error) {
		return nil, wantErr
	}
