...
func TestGetVolumeCapacityByPersistentVolumeName(t *testing.T) {
	...
	got, err := getVolumeCapacity(context.Background(), client, "s3-csi", "pvc-legacy")
	if err != nil {
...
func TestGetVolumeCapacityByCSIVolumeHandle(t *testing.T) {
	...
	got, err := getVolumeCapacity(context.Background(), client, "s3-csi", "/buckets/pvc-1234")
	if err != nil {
		t.Fatalf("getVolumeCapacity: %v", err)
	}
...
func TestGetVolumeCapacityRejectsDuplicateHandles(t *testing.T) {
	...
	if _, err := getVolumeCapacity(context.Background(), client, "s3-csi", "/shared/path"); err == nil {
...
func TestGetVolumeCapacityScopesHandleToDriver(t *testing.T) {
	...
	client := fake.NewSimpleClientset(
		otherDriverVolume,
		newPersistentVolume("s3-pv", "/shared/path", "2Gi"),
	)

	got, err := getVolumeCapacity(context.Background(), client, "s3-csi", "/shared/path")
	if err != nil {
		t.Fatalf("getVolumeCapacity: %v", err)
	}
...
func TestGetVolumeCapacityIgnoresNameCollisionOnFastPath(t *testing.T) {
	...
	got, err := getVolumeCapacity(context.Background(), client, "s3-csi", "/mnt/data")
	if err != nil {
		t.Fatalf("getVolumeCapacity: %v", err)
	}
